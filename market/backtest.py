...
from .strategy import Strategy, StockSignal, SignalType
    ...
    def _run_static(
        ...
        **kwargs,
        ...
        for date in trading_dates:
            signals = self._get_signals_for_date(date, all_data, strategy, symbols)
        ...
        **kwargs,
        ...
        for idx, date in enumerate(trading_dates):
            ...
            signals = self._get_signals_for_date(
                date, all_data, strategy, list(all_data.keys())
            )
            self._process_sell_signals(date, signals)
            self._process_buy_signals(date, signals, position_size, max_positions)
            self._update_positions(date, all_data)
